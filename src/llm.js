/**
 * LLM interaction logic using OpenRouter API directly.
 * @module llm
 */
  ...
  while (attempt < MAX_RETRIES) {
    try {
      ...
      if (!response.ok) {
        ...
        const isCorruptedThought = response.status === 400 && errorText.includes("Corrupted thought signature");
        
        if (isTransient || isCorruptedThought) {
          console.warn(`Attempt ${attempt + 1} failed: ${response.status} - ${errorText}. Retrying...`);
          attempt++;
      ...
      if (error.name === 'TypeError' && error.message === 'fetch failed') {
         console.warn(`Attempt ${attempt + 1} failed: Network error. Retrying...`);
         attempt++;
         if (attempt < MAX_RETRIES) {
